...
var url = "/" + repo_dir + "/tables/bias.xml";
        $.ajax({
			...
			$(xml).find('study').each(function(){
					...
					studysize += eval($(this).find('citation').attr('studysize'))
			...
			$("#rationale").html("\'Most information (<span style='color:red;font-weight:bold'>" + eval(1 - $(xml).find("study:contains(Unclear)").length/denom).toFixed(2) + " or more</span> - " + studysize " subjects) is from studies at low risk of bias.\' (<a href=\'http://handbook.cochrane.org/chapter_8/table_8_7_a_possible_approach_for_summary_assessments_of_the.htm\'>Cochrane Handbook</a>)");
			if (ratio > 0.5){
